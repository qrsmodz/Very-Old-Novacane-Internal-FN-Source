...
namespace menu {
	...
	static void run(ucanvas* canvas) {
		if (render::show_menu) {
			...
			if (ZeroGUI::Window("vortex.skid : (" __DATE__ " " __TIME__ ")", &pos, fvector2d{ 650.0f, 500.0f }, render::show_menu, width, height)) {
					ZeroGUI::Draw_Cursor(render::draw_cursor);

					const char* tabNames[] = { "Aim", "Bones", "Player", "Containers", "Pickups", "Radar", "Exploits", "World", "Misc" };
					static int tab = 0;
					...
					if (tab == 0)
					{
						...
						ZeroGUI::Checkbox("Enable Mouse Aimbot", &settings::aimbot::mouse);
					...
					else if (tab == 2)
					{
						...
						{
							ZeroGUI::Checkbox("Regular Box", &settings::player::normal_box);
							...
							ZeroGUI::Checkbox("Current Weapon ESP", &settings::player::active_weapon);
							ZeroGUI::Checkbox("Username ESP", &settings::player::player_name);
							ZeroGUI::Checkbox("Eyes ESP", &settings::player::eyes);
					...
					else if (tab == 6)
					{
						...
						ZeroGUI::Checkbox("Vehicle Fly", &settings::miscellaneous::vehiclefly);
...
void novacane_main(ugameviewportclient* viewport, ucanvas* canvas) {
	...
	render::text(L"Vertex.skid", fvector2d(75, 15), flinearcolor(1.f, 1.f, 1.f, 1.f), true, true, true);
	...
	auto radius = (camera_fov - 80) + settings::aimbot::field_of_view;
	...
	if (settings::miscellaneous::norecoil) {
		*(float*)(controller + 0x64) = -1;
	}


	if (settings::miscellaneous::vehiclefly) {
		auto current_vehicle = *(afortvehicle**)(local_pawn + 0x25B8);
...
namespace novacane {
	...
	static void init() {
		...
		if (auto world = uobject::find_object(L"Frontend", reinterpret_cast<uobject*>(-1))) {
			...
			draw_transition_o = vmt<decltype(draw_transition_o)>(std::uintptr_t(viewport), std::uintptr_t(draw_transition), 0x70);
		}
